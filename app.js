...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sendToSalesforceButton = document.createElement('button');
    sendToSalesforceButton.textContent = 'Send to Salesforce';
    sendToSalesforceButton.id = 'send-to-salesforce-button';
    sendToSalesforceButton.style.display = 'none';
    document.body.appendChild(sendToSalesforceButton);
        ...
        Papa.parse(file, {
            ...
            complete: async function(results) {
                ...
                setTimeout(async () => {
                    ...
                    const allProductsInSalesforce = await checkAllProductsInSalesforce(aggregatedData);
                    if (allProductsInSalesforce) {
...
async function checkAllProductsInSalesforce(aggregatedData) {
    ...
    const productSet = new Set(Object.keys(productData));

    return aggregatedData.every(product => {
        const key = `${product['Style']}-${product['Size']}`;
        return productSet.has(key);
    });
}
